...
export default function AdventurePage() {
  ...
  useEffect(() => {
    _supabase.from('cafes').select('*').eq('category', 'adventure').order('area', { ascending: true })
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      document.querySelectorAll('.adv-card').forEach(card => observer.observe(card));
    }, 300);
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .page-hero-bg{position:absolute;inset:-5%;background:url('https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=1600&q=80') center/cover;filter:brightness(0.25) saturate(0.8);z-index:0}
        ...
        .adv-card{border-radius:8px;overflow:hidden;background:#0d0d0d;border:0.5px solid rgba(255,255,255,0.07);cursor:pointer;opacity:0;transform:translateY(24px);transition:opacity 0.5s ease,transform 0.5s ease}
        .adv-card.visible{opacity:1;transform:translateY(0)}
        .adv-card:hover{border-color:rgba(255,107,0,0.4);transform:translateY(-5px);box-shadow:0 20px 48px rgba(0,0,0,0.5)}
       .adv-img{height:170px;overflow:hidden;position:relative}
        .adv-img img{width:100%;height:100%;object-fit:cover;transition:transform 0.5s}
        .adv-card:hover .adv-img img{transform:scale(1.06)}
        .adv-img::after{content:'';position:absolute;bottom:0;left:0;right:0;height:60%;background:linear-gradient(to top,rgba(0,0,0,0.75) 0%,transparent 100%);pointer-events:none;z-index:1}
        .adv-tag-pill{position:absolute;top:10px;left:10px;font-size:8px;letter-spacing:0.1em;text-transform:uppercase;background:rgba(0,0,0,0.75);border:0.5px solid rgba(255,255,255,0.15);padding:4px 8px;border-radius:2px;color:rgba(255,255,255,0.7);backdrop-filter:blur(8px)}
        .adv-rating{position:absolute;top:10px;right:10px;font-size:9px;background:#FF6B00;padding:3px 8px;border-radius:2px;color:#fff;font-weight:500}
        .adv-info{padding:14px}
        .adv-area{font-size:8px;letter-spacing:0.16em;text-transform:uppercase;color:#FF6B00;margin-bottom:4px}
        .adv-name{font-family:'Bebas Neue',sans-serif;font-size:20px;letter-spacing:0.04em;color:#fff;margin-bottom:4px}
        .adv-desc{font-size:10px;color:rgba(255,255,255,0.3);line-height:1.6;margin-bottom:10px}
        .adv-footer{display:flex;align-items:center;justify-content:space-between;border-top:0.5px solid rgba(255,255,255,0.06);padding-top:10px}
        .adv-price{font-size:13px;color:#fff;font-weight:500}
        .adv-price span{font-size:9px;color:rgba(255,255,255,0.3);font-weight:400}
        .adv-book{font-size:9px;letter-spacing:0.1em;text-transform:uppercase;color:#FF6B00;border:0.5px solid rgba(255,107,0,0.35);padding:5px 10px;border-radius:2px;background:transparent;cursor:pointer;transition:all 0.2s;font-family:'DM Sans',sans-serif}
        .adv-book:hover{background:#FF6B00;color:#fff}
     ...
     <Navbar active="adventure" />
      ...
      <div className="page-hero">
        ...
        <div className="page-hero-inner">
          <div className="hero-breadcrumb">Home <span>/ Adventure</span></div>
          <div className="hero-title">Seek <span>Adventure</span><br />Near Pune</div>
          <div className="hero-meta"><b>{cards.length}</b> activities listed · Trekking · Camping · Water Sports</div>
        </div>
      </div>
      ...
      <div className="search-section">
        <div className="search-bar">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search activities, locations..." />
        </div>
      ...
      <div className="page-layout">
        <aside className="sidebar">
          <div className="sb-title">Activity Type</div>
          {['Trekking','Camping','Rock Climbing','Water Sports','Paragliding','Cycling','Safari','Bungee'].map(a => (
            <div key={a} className="sb-option">
              <div className="sb-opt-l"><div className="sb-check" />{a}</div>
            </div>
          ))}

          <div className="sb-title">Distance from Pune</div>
          {['< 50 km','50–100 km','100–200 km','200 km+'].map(d => (
            <div key={d} className="sb-option">
              <div className="sb-opt-l"><div className="sb-check" />{d}</div>
            </div>
          ))}

          <div className="sb-title">Budget per Head</div>
          <div className="price-row"><span>₹500</span><span>₹10,000</span></div>
          <input type="range" className="slider" min={500} max={10000} defaultValue={3000} />

          <div className="sb-title">Difficulty</div>
        ...
        <main className="main-content">
          <div className="content-header">
            <div className="results-count"><b>{displayed.length}</b> adventures found</div>
          </div>
          ...
          <div className="cards-grid">
            {loading ? (
              ...
              <div className="empty-state">
                ...
                <div className="empty-text">No Adventures Found</div>
                <div className="empty-sub">Try a different search or check back later</div>
              </div>
            ) : displayed.map((d, i) => (
              <div key={i} className="adv-card" onClick={() => openBooking(d)}>
                <div className="adv-img">
                  {d.image_url && <img src={d.image_url} alt={d.name} />}
                  <div className="adv-tag-pill">{d.tag || `Adventure · ${d.area}`}</div>
                  {d.rating && <div className="adv-rating">⭐ {d.rating}</div>}
                </div>
                <div className="adv-info">
                  <div className="adv-area">{d.area}</div>
                  <div className="adv-name">{d.name}</div>
                  <div className="adv-desc">{d.description || 'An exciting adventure awaits you'}</div>
                  <div className="adv-footer">
                    <div className="adv-price">{d.price} <span>/person</span></div>
                    <button className="adv-book">Book Now →</button>
                  </div>
                </div>
              </div>
            ))}
